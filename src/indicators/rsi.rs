...
//     period: u32,
//     values: Vec<(String, Vec<Candlestick>)>,
// ) -> Vec<(String, Vec<f64>)> {
//     values
//         .into_iter()
//         .map(|(s, c)| {
//             (
//                 s,
//                 rsi(period, &c.into_iter().map(|c| c.close_price).collect()),
//             )
//         })
//         .collect()
// }

// pub fn rsi_from_chart_data(
//     period: u32,
//     values: Vec<(String, Result<Vec<Candlestick>, TrailerError>)>,
// ) -> Vec<(String, Vec<f64>)> {
//     values
//         .into_iter()
//         .filter(|(_s, c)| c.is_ok())
...
//                 rsi(
//                     period,
//                     &c.unwrap().into_iter().map(|c| c.close_price).collect(),
//                 ),
//             )
//         })
//         .collect()
